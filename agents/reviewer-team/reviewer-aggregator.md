...
2. Structured JSON review output from each specialist agent (conforming to `.claude/templates/review-schema.json`)
3. The original PR metadata (title, description) or codebase review context
4. **Addressed findings list** (optional, PR reviews only) — a list of review threads from previous AI reviews that have been resolved or acknowledged by the PR author. Each entry contains:
...
Return a single JSON code block conforming to `.claude/templates/review-aggregate-schema.json`. **No text outside the JSON block.**
...
- **`review_type`**: `"pr"` for pull request reviews, `"codebase"` for full codebase reviews
- **`pr`** metadata: Include whatever PR info is available (omit for codebase reviews)
- **`summary.suppressed_as_addressed`**: Count of findings dropped because they matched previously addressed review threads (0 if none, omit for codebase reviews)
