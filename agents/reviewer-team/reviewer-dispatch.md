...
All specialist agents produce structured JSON output conforming to `.claude/templates/review-schema.json`. Each agent returns a JSON object with `version`, `agent`, `summary`, and `comments` fields. The aggregator agent (`reviewer-aggregator`) parses these outputs to deduplicate and aggregate findings. See `.claude/templates/review-output-format.md` for the complete schema reference.
